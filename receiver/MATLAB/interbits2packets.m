function num_bits = interbits2packets(bits, limits, blank, parameter, filename)
...
if(blank)
    ...
    if(pt2)
        fseek(fID, (parameter.szbits-1)*12, 'bof');
    end
    ...
    while idx2 < length(bits) - parameter.szbits-4
        ...
        if(mod(idx3-1,2) == pt2 && done == 1 && (bits(idx2) == 1 && bits(idx2+1) == 0 && bits(idx2+2) == 1 && bits(idx2+3) == 0))
    ...
    pt2 = contains(filename, pattern);
    if(pt2)
        fseek(fID, (parameter.szbits-1)*13, 'bof');
    end
